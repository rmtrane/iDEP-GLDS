...
geneBarPlotError <- function(convertedData.out, allGeneInfo.out, input_selectOrg, input_geneSearch, input_useSD='FALSE') {
  x <- convertedData.out
...
heatmapPlotly <- function (convertedData.out, heatColors, allGeneInfo.out, input_geneCentering=TRUE, input_sampleCentering=FALSE, input_geneNormalize=FALSE, input_sampleNormalize=FALSE, input_heatColors1=1) {
...
PCAplot <- function(convertedData.out, readSampleInfo.out, input_selectFactors, input_selectFactors2) {   #PCA
  x <- convertedData.out;
...
MDSplot <- function(convertedData.out, readSampleInfo.out, input_selectFactors, input_selectFactors2) {  # MDS
  x <- convertedData.out;
...
tSNEplot <- function(convertedData.out, readSampleInfo.out, input_selectFactors, input_selectFactors2, input_tsneSeed2) {  # t-SNE
  x <- convertedData.out;
...
#Distribution of SDs
distributionSD <- function(convertedData.out, input_nGenesKNN=2000) {
...
KmeansNclusters <- function(convertedData.out, input_nGenesKNN=2000) { # Kmeans clustering
...
Kmeans <- function(convertedData.out, maxGeneClustering = 12000, input_nGenesKNN=2000, input_nClusters=4, input_kmeansNormalization='geneMean', input_KmeansReRun=0) { # Kmeans clustering
...
# main function
limma <- function(convertedData.out, readSampleInfo.out, input_dataFileFormat = 1, input_countsLogStart = 4, convertedCounts.out=convertedCounts.out, input_CountsDEGMethod=3, input_limmaPval=0.1, input_limmaFC=2, input_selectModelComprions, input_selectFactorsModel, input_selectInteractions=NULL, input_selectBlockFactorsModel=NULL, factorReferenceLevels.out) {
  if(input_dataFileFormat == 1 ) {  # if count data
    if(input_CountsDEGMethod == 3 ) {    # if DESeq2 method
      ...
      return(   DEG.DESeq2(rawCounts =  convertedCounts.out,
                           maxP_limma = input_limmaPval, 
                           minFC_limma = input_limmaFC,
                           selectedComparisons = input_selectModelComprions, 
                           sampleInfo = readSampleInfo.out,
                           modelFactors = c(input_selectFactorsModel,input_selectInteractions),
    ...
    if(input_CountsDEGMethod < 3 )    # voom or limma-trend
      return( DEG.limma(x = convertedData.out, 
                        maxP_limma = input_limmaPval, 
                        minFC_limma = input_limmaFC,
                        rawCounts = convertedCounts.out, 
                        countsDEGMethods = input_CountsDEGMethod, 
                        priorCounts = input_countsLogStart,
                        dataFormat = input_dataFileFormat,
                        selectedComparisons = input_selectModelComprions, 
                        sampleInfo = readSampleInfo.out,
                        modelFactors = c(input_selectFactorsModel,input_selectInteractions),
                        blockFactor = input_selectBlockFactorsModel) )
...
selectedHeatmap.data <- function(convertedData.out, readSampleInfo.out, limma.out, .converted.out, .readData.out, .input_noIDConversion, input_dataFileFormat, input_CountsDEGMethod, input_selectModelComprions, input_selectFactorsModel, factorReferenceLevels.out, input_selectContrast){
  ...
  iz = findContrastSamples(selectContrast = input_selectContrast,
                           allSampleNames = colnames(convertedData.out),
                           sampleInfo = readSampleInfo.out,
                           selectFactorsModel = input_selectFactorsModel,
                           selectModelComprions = input_selectModelComprions,
                           referenceLevels = factorReferenceLevels.out,
                           countsDEGMethod = input_CountsDEGMethod,
                           dataFileFormat = input_dataFileFormat
  )
...
STRINGdb_geneList <- function(geneListData.out, findTaxonomyID.out) {
  ...
  #Intialization
  string_db <- STRINGdb$new( version="11", species=taxonomyID,
...
selectedPathwayHeatmap <- function(mycolors=mycolors, heatColors, input_heatColors1=1) {
...
biclustering <- function(convertedData.out, input_nGenesBiclust=1000, input_biclustMethod='BCCC()'){
...
wgcna <- function (convertedData.out, maxGeneWGCNA = 2000, input_mySoftPower=5, input_nGenesNetwork=1000, input_minModuleSize=20 ){
...
moduleNetwork <- function(wgcna.out, input_noIDConversion, allGeneInfo.out, input_selectOrg, input_selectWGCNA.Module="Entire network", input_topGenesNetwork=10, input_edgeThreshold=0.4, input_selectGO5){
  ...
  if( tolower(input_selectWGCNA.Module) == "entire network")
